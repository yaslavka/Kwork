...
function BusinessLifeCategory({listItem}) {
    const [it, setIt]=useState(false)
    return (
        <>
            <li className={'js-cat-menu-thin-item'} onClick={()=>setIt(false)} onMouseOver={()=>setIt(true)} onMouseOut={()=>setIt(false)}>
                <Link to={`${listItem.business.route}`} className={it ?'js-category-menu-item category-menu__list-item category-menu__list-item--hover':'js-category-menu-item category-menu__list-item' }>
                    <span className="js-cat-menu-thin-span category-menu__list__inner category-menu__list__inner--new">{listItem.business.label}</span>
                </Link>
                <div style={{opacity: 1, display: it?"block":"none"}} className={'menubox menubox-all'}>
                    <div className={'menulist'} style={{left: '-300px'}}>
                        <div className={'submenu submenu--new'}>
                            {listItem.business.col1 && (
                                <div className={"submenu-column submenu-column--cols-3"}>
                                    {listItem.business.col1.map((col1)=>(
                                        <>
                            ...
                            {listItem.business.col2 && (
                                <div className={"submenu-column submenu-column--cols-3"}>
                                    {listItem.business.col2.map((col2)=>(
                                        <>
                                            <div className={'menutitle'}>
                            ...
                            {listItem.business.col3 && (
                                <div className={"submenu-column submenu-column--cols-3"}>
                                    {listItem.business.col3.map((col3)=>(
                                        <>
                                            <div className={'menutitle'}>
