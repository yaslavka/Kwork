...
function DesignCategory({listItem}) {
    ...
    return(
        <>
            <li className={'js-cat-menu-thin-item'} onClick={()=>setDesign(false)} onMouseOver={()=>setDesign(true)} onMouseOut={()=>setDesign(false)}>
                <Link to={`${listItem.design.route}`} className={design ?'js-category-menu-item category-menu__list-item js-cat-menu-thin-link category-menu__list-item--hover':'js-category-menu-item category-menu__list-item js-cat-menu-thin-link' }>
                    <span className="js-cat-menu-thin-span category-menu__list__inner category-menu__list__inner--new">{t(listItem.design.label)}</span>
                </Link>
                                                    ...
                                                    {col1.subCategory.map((subCategory)=>(
                                                        <>
                                                            <Link to={subCategory?.route} className={'submenu-item'}>
                                                                <span className="submenu-item__text js-menu-item" onClick={()=>setDesign(false)}>{subCategory.label}</span>
                                                                {subCategory.icon ? (
                                                                    <>
                                ...
                                <div className={"submenu-column submenu-column--cols-3"}>
                                    {listItem.design.col3.map((col3)=>(
                                        <>
                                            ...
                                            <div className={'submenu-group-wrapper'}>
                                                <div className={'submenu-group-inner'}>
                                                    {col3.subCategory.map((subCategory)=>(
                                                        <>
                                                            <Link to={subCategory?.route} className={'submenu-item'}>
                                                                <span className="submenu-item__text js-menu-item" onClick={()=>setDesign(false)}>{subCategory.label}</span>
                                                                {subCategory.icon ? (
