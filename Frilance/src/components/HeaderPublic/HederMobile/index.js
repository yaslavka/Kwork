...
function HeaderMobile ({setModalAuth, setModalSignUp}) {
    ...
    return(
        <>
            {/*mapMenu*/}
            {menu && (
                <MobileMenu menu={menu} setMenu={setMenu} setModalAuth={setModalAuth} setModalSignUp={setModalSignUp}/>
            )}
            <div className={'headertop-mobile m-visible '}>
                ...
                <div className="header__logo">
                    <img className="js-header-theme-logo"
                         src={headerlogo}
                         height="22" width="105" alt="mobile logo"/>
                </div>
                <a className="menu-category menu-category_login login-js" onClick={()=>{setModalAuth(true); document.getElementById('body').classList.add('modal-open')}}>Войти</a>
            </div>
        </>
    )
}
